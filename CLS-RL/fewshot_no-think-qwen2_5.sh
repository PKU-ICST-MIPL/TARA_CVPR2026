...
export LOG_PATH="debug_log_qwen2.5vl-3b-instruct-no-thinking_${DATASET}-1shot-fewshot-ep${NUM_TRAIN_EPOCHS}.txt"
OUTPUT_DIR="../../checkpoints/Qwen2.5-VL-3B-Instruct-no-thinking-${DATASET}-1shot-fewshot-ep${NUM_TRAIN_EPOCHS}/"
...
PYTHONIOENCODING=utf-8 python -m torch.distributed.run --nproc_per_node="8" \
    ...
    --run_name "Qwen2.5-VL-3B-Instruct-${DATASET}-1shot-fewshot-ep${NUM_TRAIN_EPOCHS}" \
    --save_steps 100 \
