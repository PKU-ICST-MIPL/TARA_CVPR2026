    ...
    def __init__(
        ...
        if is_deepspeed_zero3_enabled():
            if "qwen3-vl" in model_id_lower:
                model_init_kwargs.pop("use_cache", None)
                if "-a" in model_basename:
                    self.ref_model = Qwen3VLMoeForConditionalGeneration.from_pretrained(model_id, torch_dtype=torch.bfloat16, **model_init_kwargs)
                else:
                    self.ref_model = Qwen3VLForConditionalGeneration.from_pretrained(model_id, torch_dtype=torch.bfloat16, **model_init_kwargs)
            elif "qwen2.5-vl" in model_id_lower:
